...
{  
    ...
    function smtp_check(){
        ...
        $f = @fsockopen($apdomain, 25); // Attempt to open a socket connection on port 25 (SMTP)
        
        if ($f !== false) { // Check if the socket connection was successful
            $res = fread($f, 1024); // Read the response from the server
            
            if (strlen($res) > 0 && strpos($res, '220') === 0) {
                @fclose($f); // Close the socket if it's open and the response is valid
                return TRUE; // SMTP server is reachable and responding correctly
            } else {
                @fclose($f); // Close the socket if the response is invalid
                return FALSE; // SMTP server is not responding correctly
            }
        }
        
        // If $f is false, no need to call fclose()
        return FALSE; // SMTP server is not reachable
    }
